 ...
 * draws it — as a half-section, the upper half of the profile, landscape (8 : 5). The current
 * setup is dashed, the new one solid, and three figures say what moved — the outer rim edge, the
 * inner rim edge and the tyre's outer diameter.
 *
 ...
 *
 * Every line is a 1px hairline (`vector-effect: non-scaling-stroke`) in ink, ink-2 or ink-3 —
 * never blue; blue is for actions. Dimension lines end in square ticks, drawing-office style, not
 * arrowheads. Labels are HTML placed over the drawing by viewBox percentages, so they stay at the
 * micro size at every width instead of scaling with the sheet.
 *
...
/* The profiles are cut off at the sheet's bottom edge, half-section style; the id is per instance, stable on both sides. */
const clipId = `offset-sheet-${useId()}`

/* ── The sheet, in viewBox units (8 : 5) ─────────────────────────────────────── */

const VB_W = 480
const VB_H = 300
/** The mounting face. It never moves. Inboard has the larger share of the sheet: ET is mostly positive. */
const FACE_X = 292
/** The box the profiles may fill; the margins around it belong to dimension lines and labels. */
const FIT = { left: 36, right: 444, top: 46, bottom: 258 }
const DIM_Y = 26
const TICK = 3
const OVERSHOOT = 4
const GAP = 2
const LABEL_MIN_PCT = 12
const LABEL_MAX_PCT = 78
/** How much of the disc shows under the lower rim's well before the sheet ends (mm). */
const DISC_BELOW_MM = 24
...
/** One wheel as mm coordinates: x from the mounting face (outboard positive), r from the axle. */
...
/** The radius the sheet's bottom edge stands for: a little disc under the lower of the two wells. */
function baseRadius(a: Section, b: Section): number {
    return Math.min(a.rRim, b.rRim) - WELL_D - DISC_BELOW_MM
}
...
function scaleFor(a: Section, b: Section): number {
    ...
    return Math.min(
        ...
        (FIT.bottom - FIT.top) / Math.max(rMax - baseRadius(a, b), 1)
    )
}

type Point = (x: number, r: number) => string

function mapper(scale: number, rBase: number): { X: (x: number) => number; Y: (r: number) => number; p: Point } {
    const X = (x: number) => Math.round((FACE_X + x * scale) * 10) / 10
    const Y = (r: number) => Math.round((FIT.bottom - (r - rBase) * scale) * 10) / 10

    return { X, Y, p: (x, r) => `${X(x)} ${Y(r)}` }
}

/* The profiles, upper half only: what a half-section shows. */

function rimPath(g: Section, p: Point): string {
    return (
        `M${p(g.edgeIn, g.rRim - 8)} L${p(g.edgeIn, g.rFlange)} L${p(g.xi, g.rFlange)} ` +
        `L${p(g.xi, g.rRim)} L${p(g.xi + 22, g.rRim)} L${p(g.xi + 38, g.rRim - WELL_D)} ` +
        `L${p(g.xo - 58, g.rRim - WELL_D)} L${p(g.xo - 40, g.rRim)} L${p(g.xo, g.rRim)} ` +
        `L${p(g.xo, g.rFlange)} L${p(g.edgeOut, g.rFlange)} L${p(g.edgeOut, g.rRim - 8)}`
    )
}
...
function discPath(g: Section, p: Point): string {
    ...
    return (
        `M${p(0, HUB_BORE)} L${p(PAD_T, HUB_BORE)} ` +
        `M${p(0, HUB_BORE)} L${p(0, HUB_R)} L${p(g.xo - 52, ramp(g.xo - 52))} ` +
        `M${p(PAD_T, HUB_BORE)} L${p(PAD_T, HUB_R)} L${p(g.xo - 42, ramp(g.xo - 42))}`
    )
}
...
function tyrePath(g: Section, p: Point): string {
    ...
    return (
        `M${p(g.edgeIn, g.rFlange)} Q${p(xl, root)} ${p(xl, mid)} ` +
        `Q${p(xl, g.rTyre)} ${p(xl + shoulder, g.rTyre)} L${p(xr - shoulder, g.rTyre)} ` +
        `Q${p(xr, g.rTyre)} ${p(xr, mid)} Q${p(xr, root)} ${p(g.edgeOut, g.rFlange)}`
    )
}

/* ── Dimension lines: extension lines, the line itself, and square ticks at its ends ─ */
...
/** A horizontal dimension above two features at `y`; the extension lines rise from the features. */
function hDim(x1: number, fromY1: number, x2: number, fromY2: number, y: number): Dimension {
    const lo = Math.min(x1, x2)
    const hi = Math.max(x1, x2)

    return {
        ext: `M${x1} ${fromY1 - GAP} L${x1} ${y - OVERSHOOT} M${x2} ${fromY2 - GAP} L${x2} ${y - OVERSHOOT}`,
        line: `M${lo} ${y} L${hi} ${y} M${lo} ${y - TICK} L${lo} ${y + TICK} M${hi} ${y - TICK} L${hi} ${y + TICK}`,
    }
}
...
const drawing = computed(() => {
    ...
    const { X, Y, p } = mapper(scaleFor(a, b), baseRadius(a, b))
    const rMax = Math.max(a.rTyre, b.rTyre)
    const outer = hDim(X(a.edgeOut), Y(a.rFlange), X(b.edgeOut), Y(b.rFlange), DIM_Y)
    const inner = hDim(X(a.edgeIn), Y(a.rFlange), X(b.edgeIn), Y(b.rFlange), DIM_Y)
    ...
    return {
        ...
        disc: discPath(b, p),
        tyre: tyrePath(b, p),
        face: `M${FACE_X} ${Y(rMax) - 8} L${FACE_X} ${FIT.bottom}`,
        centre: `M${X(b.xc)} ${Y(b.rFlange) - 8} L${X(b.xc)} ${FIT.bottom}`,
        ext: `${outer.ext} ${inner.ext}`,
        dims: `${outer.line} ${inner.line}`,
    }
})
...
const labels = computed(() => {
    ...
    return {
        ...
        /* At the right edge just under the tread line — clear of the "außen" figure above the dimension line. */
        diameter: {
            text: `Ø${NNBSP}${mm(r.next.diameterMm)}`,
            top: pct(Y(b.rTyre) + 2, VB_H),
        },
        face: {
            left: pct(FACE_X, VB_W),
            top: pct(FIT.bottom + 6, VB_H),
        },
    }
})
    ...
    <div ref="root" class="offset">
        <div class="offset__figure" role="img" :aria-label="description">
            <svg class="offset__svg" :viewBox="`0 0 ${VB_W} ${VB_H}`" aria-hidden="true" focusable="false">
                <defs>
                    <clipPath :id="clipId">
                        <rect x="0" y="0" :width="VB_W" :height="FIT.bottom" />
                    </clipPath>
                </defs>
            ...
            <span class="offset__label offset__label--face micro" :style="{ left: labels.face.left, top: labels.face.top }">
                Anlagefläche
            </span>
        </div>
...
/* The sheet holds its 8 : 5 box before the drawing paints. */
.offset__figure {
    position: relative;
    width: 100%;
    aspect-ratio: 8 / 5;
...
/* Dimension lines and their figures in the secondary ink; nothing in the drawing is blue. */
.offset__dim {
    stroke: var(--c-ink-2);
}
